...
const ArticleCard: React.FC<{ article: NewsArticle }> = ({ article }) => (
  <div className="bg-white rounded-md overflow-hidden flex flex-col group border border-gray-200 h-full tilt-card hover:shadow-xl transition-shadow">
    ...
    <div className="p-6 flex flex-col flex-grow">
      <p className="text-sm text-brand-accent mb-2 font-semibold uppercase tracking-wider font-body">{article.date}</p>
      <h3 className="text-xl font-bold font-display text-brand-heading mb-3 flex-grow group-hover:text-brand-accent transition-colors">{article.title}</h3>
      <p className="text-brand-gray text-base mb-6 font-body">{article.excerpt}</p>
      <a href="#" className="font-semibold text-brand-accent hover:text-brand-accent-600 self-start mt-auto font-body transition-colors">
        Beitrag lesen &rarr;
      </a>
    </div>
  </div>
