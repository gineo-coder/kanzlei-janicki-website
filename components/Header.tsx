...
const Header: React.FC = () => {
  ...
  const navLinkClasses = `block py-2 px-3 rounded md:bg-transparent md:p-0 transition-colors duration-300 font-medium md:text-lg ${
    isScrolled || !isOnHomePage ? 'text-brand-primary hover:text-brand-accent' : 'text-white hover:text-gray-200'
  }`;
    ...
    <header className={`fixed w-full z-20 top-0 left-0 transition-all duration-300 ${isScrolled ? 'bg-white/95 backdrop-blur-lg shadow-md' : 'bg-transparent'}`}>
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div className="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
          ...
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} type="button" className={`inline-flex items-center p-2 w-10 h-10 justify-center text-sm rounded-lg md:hidden hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-gray-400 ${isScrolled || !isOnHomePage ? 'text-brand-primary' : 'text-white'}`} aria-controls="navbar-sticky" aria-expanded={isMenuOpen}>
